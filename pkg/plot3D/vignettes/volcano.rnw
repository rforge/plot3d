...
\Keywords{
  volcano, persp3D, Image, plot3D, \proglang{R}
}

\Plainkeywords{
  volcano, persp3D, Image, plot3D, R
}
...
%\VignetteKeywords{persp3D, Image, Volcano, plot3D}
...
\section{Intro}
To make this vignette smaller, I reduce the size of \code{volcano}:
<<>>=
...
contour(Volcano)
Contour(Volcano, lwd = 2)
image(Volcano)
image(Volcano, col = jet.col(100))
Image(Volcano, clab = "m")
Image(Volcano, facets = FALSE)
Image(Volcano, contour = TRUE)
Image(Volcano, contour = list(lwd = 2, col = jet.col(11)))
Image(Volcano, theta = 30, NAcol = "black")
@
...
@
\end{center}
\caption{The Image and contour function}
...
persp3D(z = Volcano, facets = FALSE, shade = 0.2)
...
persp3D(z = Volcano, col = "lightblue", shade = 0.5, bty = "bl")
...
ribbon3D(z = Volcano[ix, ], dir = "y")
ribbon3D(z = Volcano[ix, iy], dir = "xy")
...
persp3D(z = Volcano, clab = c("height", "m"),
      colkey = list(length = 0.5, dist = -0.1, shift = -0.1))
par(mar = c(4, 4, 2, 2))
Image(z = Volcano, clab = "height, m",
      colkey = list(dist = -0.20, shift = 0.15,
      side = 3, length = 0.5, width = 0.5,
      cex.clab = 1.2, col.clab = "white", col.axis = "white",
...
@
\end{center}
\caption{combined persp3D and Image}
\label{fig:comb}
\end{figure}
...
# draw the volcano, with contours at bottom 
 persp3D (x, y, z = volcano, theta = 10, phi = 20, 
          box = FALSE, scale = FALSE, expand = 0.3, 
          contour = TRUE, zlim = c(0, 200), 
...
# add a plane (image) at z = 170; jetcolored, transparant: only border
 image3D(x, y, z = 170, add = TRUE, clim = range(volcano),
         colvar = volcano, colkey = FALSE, 
         facets = NA, plot = FALSE)

# add a contour (image) at z = 170; jetcolored, 
 contour3D(x, y, z = 170, add = TRUE, clim = range(volcano), lwd = 3,
           colvar = volcano, facets = NA, colkey = FALSE, plot = TRUE)
...
panelfirst <- function(pmat) {
  ...
  scatter(XY$x, XY$y, colvar = Volcano[10,],
