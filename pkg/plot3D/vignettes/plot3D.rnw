...
\begin{itemize}
 \item In addition to the x, y (and z) values, there an additional data dimension 
 can be represented by a color variable (argument \code{colvar}) which defines the coloration. 
...
\begin{itemize}
 ...
 \item Other functions
  ...
  \item \code{text2D}, \code{arrows2D}, \code{segments2D}, \code{rect2D}, 
    \code{polygon2D} for other 2D functions that have a color key.
 \item Colors and colorkeys:
   \begin{itemize}
     \item \code{colkey}: color legendS.
     \item \code{jet.col}, \code{gg.col}, \code{ramp.col}: suitable color palettes.
   \end{itemize}
...
We first plot the data, with the zoomed part. The 0 m contour lines are added.
...
args(scatter3D)
@
We plot the dataset \code{quakes}, using function \code{scatter3D}.
...
dim(Oxsat$val)
@
We use \code{image2D.array} to plot several depth intervals at once, looping over
the first and second margin:
<<label=image2D,include=FALSE>>=
image2D(z = Oxsat$val, subset = 1:8, 
