...
\begin{itemize}
 \item In addition to the x, y (and z) values, there is a color variable 
   ...
   \footnote{For \code{image2D} and \code{contour2D} this color variable is called 'z',
...
\begin{itemize}
 ...
 \item Functions defined on the \code{image} or \code{contour} function:
   \begin{itemize}
     \item \code{image2D}, \code{contour2D}, for an extended function to visualise 2-D or 3-D data.
