...
createsurfs <- function (x, y, z, colvar, names = c("x", "y", "z")) {  
...
image3D <- function(x = NULL, y = NULL, z = NULL, ..., 
  ...
  if (isconstant == 3) {
    ll <- createsurfs(x, y, z, colvar, c("x", "y", "z"))
    x <- ll$A; y <- ll$B; z <- ll$C 
  } else if (isconstant == 1) {
    ll <- createsurfs(y, z, x, colvar, c("y", "z", "x"))
    y <- ll$A; z <- ll$B; x <- ll$C 
  } else {
    ll <- createsurfs(x, z, y, colvar, c("x", "z", "y"))
    x <- ll$A; z <- ll$B; y <- ll$C 
  }
  plist <- surf3D (x = x, y = y, z = z, colvar = colvar, 
                  phi = phi, theta = theta,
                  col = col,  NAcol = NAcol, 
