...
%\VignetteKeywords{marine science, 3-D data, 4-D data, quiver, image2D}
...
This function was created to convert data from monitoring campaigns into 
...
<<label=NO3,include=FALSE, pdf = FALSE, eps = FALSE, png = TRUE, resolution = 100>>=
...
As an example, we plot the contents of the tabular NIOZ monitoring data for the 
...
Here we calculate the dataset \code{volcano}, at very low resolution. 
...
<<label=flows,include=FALSE, pdf = FALSE, eps = FALSE, png = TRUE, resolution = 100>>=
 par(mfrow = c(1, 3))
 x  <- seq(-1, 1, by = 0.2)
 y  <- seq(-1, 1, by = 0.2)
...
\begin{figure} 
\begin{center}
<<label=flows,fig=TRUE,echo=FALSE,pdf=FALSE, png = TRUE, width = 8, height = 4>>=
<<flows>>
@
\end{center}
...
It also comprises a data set with output from the Lagrangian Transport model (Ltrans) from Chesapeake Bay \citep{Ltrans}.
...
<<label=Chesa,include=FALSE, pdf = FALSE, eps = FALSE, png = TRUE, resolution = 100>>=
...
We start by plotting the geographical position at selected time points, ignoring the depth (2-D output) using traditional graphics; the color green and orange denote the source area of the particles.
Note that we can specify the bathymetric map of the area through the \code{image} argument of the function.

<<label=Ltrans2D,include=FALSE, pdf = FALSE, eps = FALSE, png = TRUE, resolution = 100>>=
lon <- Chesapeake$lon
lat <- Chesapeake$lat
...
@
\end{center}
\caption{2D distribution of particles in Chesapeake at selected time points}
\label{fig:Ltrans}
\end{figure}

In open GL, it works slightly different: first we create a 2D bathymetric map, on which we add the tracer positions. 
The output of this code is not shown, but this is very fast, so you will probably want to slow it down. 
...
<<label=Ltrans3D,include=FALSE, pdf = FALSE, eps = FALSE, png = TRUE, resolution = 100>>=
lon <- Chesapeake$lon
lat <- Chesapeake$lat
depth <- Chesapeake$depth
...
@
\end{center}
\caption{3D distribution of particles in Chesapeake at two time points}
\label{fig:Ltrans}
\end{figure}
...
The output shows only one time point: 
...
Note: there exists also a comparable function \code{movieslice3D} that creates a 
